"""Jeu de test synthetique pour le notebook de Cabouy.
...
notebook dans l'ordre, puis verifie les proprietes attendues.
    ...
    python3 tests/jeu_de_test_cabouy.py "Code pour consolider les données-Cabouy_V4.ipynb" /tmp/jdt
"""
import json
import sys
...
def executer(code, base):
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent))   # librairie ouysse
    code = list(code)
...
def verifier(espace, code, base, notebook):
    ...
    try:
        ...
        check("entree a 3 champs refusee", "colonne, motif" in str(e))
    ...
    check("cote NGF = 107.6158 + h/100",
          abs(ngf - 107.6158) < 1e-9
          and float((full["Niveau_(mNGF)"] - (ngf + h / 100)).abs().max()) < 1e-9,
    ...
    LIGNES_V2 = 907          # le notebook d'origine, retire du depot
    lignes = sum(len(c.rstrip().split("\n")) for c in code)
    check("notebook plus court que la V2", lignes < LIGNES_V2,
          f"{lignes} lignes contre {LIGNES_V2}")
    return ok


if __name__ == "__main__":
    notebook = (sys.argv[1] if len(sys.argv) > 1
                else "Code pour consolider les données-Cabouy_V4.ipynb")
